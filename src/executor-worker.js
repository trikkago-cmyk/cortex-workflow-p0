...
export function createExecutorWorker(options = {}) {
  ...
  async function handleCommand(command) {
    ...
    try {
      ...
      const replySkipped = Boolean(replyText && command.source === 'notion_comment');
      const replySkipReason = replySkipped ? 'discussion_writeback_removed' : null;
      if (replySkipped) {
        logger.info?.(
          `[executor-worker] inline discussion writeback removed for ${command.command_id}; result stays in Cortex checkpoints/docs`,
        );
      }
