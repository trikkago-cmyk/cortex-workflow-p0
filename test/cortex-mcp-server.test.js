...
test('Cortex MCP handlers forward Custom Agent events into the existing Cortex API', async (t) => {
  ...
  const green = await handlers.ingestNotionComment({
    ...
    body: '@Cortex 请继续推进 Custom Agent MCP 接入。',
...
test('Cortex MCP endpoint also supports legacy SSE clients used by some Custom Agent integrations', async (t) => {
  const dbDir = mkdtempSync(join(tmpdir(), 'cortex-mcp-sse-'));
  const cortex = createCortexServer({
    dbPath: join(dbDir, 'cortex.db'),
    clock: () => new Date('2026-04-27T09:15:00.000Z'),
  });
  ...
  const client = new Client(
    {
      name: 'cortex-mcp-test-sse-client',
      version: '0.1.0',
    },
    {
      capabilities: {},
